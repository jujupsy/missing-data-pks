# Name: data_sim.R
# Description: 
#       Funtion to simulate data for pks following the procedure from 
...
#           De Chiusole et al. (2015) or empirical KS
#           
# Output: 
#     - data_mcar.rda
#     - data_ksmnar.rda
#     - data_iksmnar.rda   # Entsprechend in einzelnen kleineren Dateien, um die 
#                            Schätzung batchweise durchzuführen (schont RAM)
#
...
data_sim <- function(NinSet, NofSets, tks = "tKS", seed = 42, cut = 100){
...
a_mu1 <- rep(c(.4, .3, .2, .1,  0), each = n_iksmnar) 
b_mu1 <- rep(c(.5, .4, .3, .2, .1), each = n_iksmnar) 
cond_iks <- rep(paste0("iks_C", 1:5), each = n_iksmnar)
set_number <- rep(1:n_iksmnar, times = 5)
a_mu0 <- rev(a_mu1)
b_mu0 <- rev(b_mu1)
